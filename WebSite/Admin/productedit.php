// Страница, на которой можно поменять информацию о товаре
// Обработка данных о продукте, изображениях, ценах и заказах: Полученные данные о продукте, его изображениях, ценах и заказах сохраняются в соответствующих переменных для дальнейшего использования в коде страницы.
// Использование полученных данных в дальнейшем коде страницы: После получения данных, они используются в дальнейшем коде страницы для отображения информации о продукте, его изображениях, ценах и заказах на странице.
    <?php
// Начало сессии: session_start() используется для инициализации работы с сессиями.
    session_start();
    if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
        // Проверка авторизации: Затем проверяется, авторизован ли пользователь. Если в сессии установлен ключ 'loggedin' и его значение равно true, это означает, что пользователь авторизован. 
        // В таком случае переменной $adminloggedin присваивается значение true, а также из сессии извлекается идентификатор пользователя $userId. 
        $adminloggedin = true;
        $userId = $_SESSION['userId'];
    } else {
        // Если пользователь не авторизован, $adminloggedin устанавливается в false, а $userId - в 0.
        $adminloggedin = false;
        $userId = 0;
    }
    ...
    if ($adminloggedin) {
    ?>
    // Открытие HTML-разметки: Если $adminloggedin равно true, открывается HTML-разметка. Это означает, что пользователь авторизован и ему доступна соответствующая страница.
        <!DOCTYPE html>
        <html lang="en">
                                                ...
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        <div class="card">
                                                            ...
                                                            <?php
        // Подключение сервиса для частичных данных о продукте: В начале кода происходит подключение (включение) файла, содержащего сервис для получения частичных данных о продукте. 
        // Этот сервис предоставляет функции для работы с базой данных и получения информации о продукте, его изображениях, ценах и заказах.
        ...
        // Получение списка каталогов: Вне зависимости от наличия или отсутствия параметра "id" в URL-адресе, также происходит получение списка каталогов с помощью функции GetCatalog().
                                                                $catalog = GetCatalog();
                                                                $product = null;
                                                                $image = null;
                                                                $prices = null;
