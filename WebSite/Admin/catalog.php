...
<?php
// Начало сессии: session_start() запускает новую сессию или возобновляет существующую, позволяя сохранять и передавать данные между различными страницами в рамках одной сессии.
// Проверка авторизации пользователя: код проверяет, авторизован ли пользователь, путем проверки существования ключа loggedin в сессии и его значения. 
// Если пользователь авторизован, устанавливается флаг $adminloggedin в true, и переменная $userId принимает значение из сессии. В противном случае, $adminloggedin устанавливается в false, а $userId в 0.
session_start();
...
if ($adminloggedin) {
?>
    // HTML-разметка страницы продукта: Весь остальной код внутри условия $adminloggedin представляет собой HTML-разметку страницы продукта. 
    // В этой разметке определены стили, скрипты, меню, таблица с категориями товаров и дополнительные функции для управления категориями.
    <!DOCTYPE html>
    <html lang="en">
                                                        ...
                                                        <div class="card-body reset-table p-t-0">
                                                            ...
                                                            <div class="table-responsive">
                                                                <table class="table table-hover">
                                                                    ...
                                                                    <tbody>
                                                                        <?php
    // Отображение списка категорий товаров: Через PHP код происходит загрузка и отображение списка категорий товаров из базы данных. 
    // Если категории существуют, они отображаются в виде таблицы с возможностью редактирования каждой категории. Если категорий нет, выводится сообщение "No Catalog".
...
<?php
// Редирект в случае неавторизованного доступа: Если пользователь не авторизован, он перенаправляется на страницу входа (index.php).
} else {
