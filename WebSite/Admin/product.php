...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
    ...
    // Если в сессии установлен ключ 'loggedin' и его значение равно true, то пользователь считается авторизованным ($adminloggedin = true), и его ID сохраняется в переменную $userId.
    $adminloggedin = true;
...
if ($adminloggedin) {
?>
// Отображение контента в зависимости от авторизации: Если пользователь авторизован, то отображается HTML-разметка для страницы продуктов. В противном случае пользователь перенаправляется на страницу входа (index.php).
// Формирование HTML-разметки страницы продуктов: В этой разметке определены стили, скрипты и содержимое страницы, такое как меню, таблица продуктов и кнопки для фильтрации и редактирования продуктов. 
// Данные о продуктах получаются из базы данных.
    <!DOCTYPE html>
    <html lang="en">
<!--  -->
    <head>
                                                        ...
                                                        <div class="card-body reset-table p-t-0">
                                                            <form action="" method="get">
                                                                <div class="form-group row"><label class="col-sm-2 col-form-label"></label>
                                                                    ...
                                                                    <div class="col-sm-4">
                                                                        <a class="btn btn-danger btn-round" style="float: right;" href="./productedit.php?id=0">Создать новый</a>
                                                                    </div>
                                                                </div>
                                                            </form>
                                                            <div class="table-responsive">
                                                                <table class="table table-hover">
                                                                    ...
                                                                    <tbody>
                                                                        <?php
// Отображение списка продуктов: Список продуктов получается из базы данных. 
// Для каждого продукта выводятся его фото, категория, наименование, описание, состав, пищевая ценность, статус и кнопка для редактирования. Если продуктов нет, выводится соответствующее сообщение.
                                                                        require_once '../src/System/DatabaseConnector.php';
                                                                        require_once 'Service/_productpartial.php';
                                                                        if (isset($_GET["filter"]))
                                                                            $all_product = GetAll($_GET["filter"]);
                                                                        else $all_product = GetAll('');

                                                                        // if ($all_product != null) {
                                                                        $num = $all_product->rowCount();
                                                                        if ($num > 0) {
                                                                            while ($row = $all_product->fetch(PDO::FETCH_ASSOC)) {
                                                                                ...
                                                                                echo '<td><div style="width: 100px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;" title="'.$row['Description'].'" >' . $row['Description'] . '</div></td>';
                                                                                echo '<td><div style="width: 100px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;" title="'.$row['Ingredients'].'" >' . $row['Ingredients'] . '</div></td>';
                                                                                echo '<td><div style="width: 100px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;" title="'.$row['NutritionalValue'].'" >' . $row['NutritionalValue'] . '</div></td>';
                                                                                if ($row['Status'] == true)
                                                                                    echo '<td><button type="button" class="btn btn-success btn-round">Активный</button></td>';
                                                                                else
                                                                                    echo '<td><button type="button" class="btn btn-danger btn-round">Скрытый</button></td>';
                                                                                echo '<td><a class="btn btn-success btn-round btn-outline-success" href="productedit.php?id=' . $row["ID"] . '">Редактировать</a></td>';

                                                                                echo '</tr>';
                                                                            }
                                                                        } else {
                                                                            echo '<tr class="ng-star-inserted">';
                                                                            echo '<td colspan="8">No Product</td>';
                                                                            echo '</tr>';
                                                                        }
                                                                        // }
                                                                        ?>
