// Страница с отчетами о продаж
<?php
// Начало сессии: Сначала запускается сессия с помощью session_start(). Сессия используется для сохранения информации о состоянии пользователя между запросами к серверу.
// Проверка авторизации: Затем код проверяет, авторизован ли пользователь. Если в сессии установлен ключ 'loggedin' и его значение равно true, то пользователь считается авторизованным. 
// В этом случае переменная $adminloggedin устанавливается в true, а также извлекается идентификатор пользователя $userId из сессии. Если пользователь не авторизован, то $adminloggedin устанавливается в false, а $userId - в 0.
session_start();
...
if ($adminloggedin) {
?>
// Отображение контента в зависимости от авторизации: Если пользователь авторизован ($adminloggedin == true), то выводится HTML-разметка страницы. В противном случае пользователь перенаправляется на страницу входа (index.php).
// Отображение информации: На странице отображается информация о количестве доставленных заказов, количестве пользователей и общем заработке. 
// Также выводится таблица с последними 10 заказами, включая ID заказа, исполнителя, статус, дату/время заказа, сумму, количество позиций и действия для редактирования заказа.
    <!DOCTYPE html>
    <html lang="en">
            ...
            <div class="pcoded-container navbar-wrapper">
                ...
                <div class="pcoded-main-container" style="margin-top: 56px;">
                    ...
                    <div class="pcoded-wrapper">
                        ...
                        <?php
                    // Использование данных из базы данных: Для получения данных, таких как количество заказов и пользователей, а также последних заказов, вероятно, используется база данных. 
                    // Функции GetAll() и _dashboardpartial.php вероятно отвечают за получение и обработку этих данных.
